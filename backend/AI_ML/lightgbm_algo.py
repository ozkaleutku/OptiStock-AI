...
def calculate_rolling_features(df_cons, until_date):
    """Calculates rolling consumption features for the core model."""
...
def run_lightgbm_training():
    ...
    # A. Prophet Tahminleri (Zaten patlatılmış verileri alıyoruz)
    ...
    # B. Stok Hareketlerinden Gerçek Tüketim
    consumption_query = """
    SELECT 
        item_id, 
    ...
    logger.info("4. Gelecek Tahminleri Yapiliyor...")
    target_year = datetime.now().year + 1
    
    # Only predict for items with history or forecast
    ...
    # 5. SONUÇLARI KAYDET
    logger.info("5. Sonuclari Kaydediliyor...")
    insert_query = """
    ...
    # Filter only hammadde for the plan table
